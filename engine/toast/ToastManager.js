...
var top_left = function(bounds) {
    return {
        x: edge_offset,
        y: edge_offset
    };
};
var top_middle = function(bounds) {
    return {
        x: (game_size.x - bounds.width) / 2,
        y: edge_offset
    };
};
var top_right = function(bounds) {
    return {
        x: game_size.x - bounds.width - 260,
        y: edge_offset
    };
};
var middle_middle = function(bounds) {
    return {
        x: (game_size.x - bounds.width) / 2,
        y: (game_size.y - bounds.height) / 2
    };
};
var middle_right = function(bounds) {
    return {
        x: game_size.x - bounds.width - edge_offset,
        y: (game_size.y - bounds.height) / 2
    };
};
var bottom_left = function(bounds) {
    return {
        x: edge_offset,
        y: game_size.y - bounds.height
    };
};
var bottom_middle = function(bounds) {
    return {
        x: (game_size.x - bounds.width) / 2,
        y: game_size.y - bounds.height
    };
};
var bottom_right = function(bounds) {
    return {
        x: game_size.x - bounds.width - edge_offset,
        y: game_size.y - bounds.height - edge_offset
    };
};

var bottom_right_attach = function(bounds) {
    return {
        x: game_size.x - bounds.width - edge_offset - 200,
        y: game_size.y - bounds.height - edge_offset
    };
};
